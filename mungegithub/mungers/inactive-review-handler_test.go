	...
	/*
		isPRActiveTests := []struct {
		...
		}{
			{
				...
				prCreatedAt: timePtr(time.Date(2017, 1, 1, 13, 5, 20, 0, time.UTC)),
				comments: []*githubapi.IssueComment{},
				reviewComments: []*githubapi.PullRequestComment{},
				expected:    false,
			},
		...
		for testNum, test := range isPRActiveTests {
			i := InactiveReviewHandler{}
			active := i.isPRActive(test.prCreatedAt, test.comments, test.reviewComments)
